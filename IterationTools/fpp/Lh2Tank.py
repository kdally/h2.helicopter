...
def designMechanicalTank(m_h2_flight, P_fc):
    time.sleep(5)
    ...
    # piping components
    t_pipe = 1e-3 # guesses
    D_outlet_in = 1e-2 * np.ones(m_h2_flight.shape) # diameter given by components, operations...
    c_discharge = 0.5 # https://www.herose.com/eng/products/cryogenic-services/06810_06815.php

    # shape parameters
    lamb = 0.6 # = l_s / r_tank sphere
    ...
    def designInnerWall(material1):
        ...
        def calculateLh2Required(m_h2_flight):
            ...
            return m_h2_flight   / (1. - c_3) #return m_h2_rq
        ...
        def calculateLh2MassToTankVolume(m_h2,p,y_max):
            ...
            #print("vol = ",m_h2/rho_mean/(1 - c_1 - c_2))
            return m_h2/(max_frac - empty_frac)/rho_mean/(1 - c_1 - c_2) #return h2 volume = V_h2
        ...
        def calcualteCylindricalWallThickness(p_vent,r_in_1,material):
            ...
            #print("t_cyl",(p_burst * r_in_1) / (weld_eff * material['stress_limit'] - 0.6 * p_burst))
        ...
        def calcualteSphericalWallThickness(p_vent,r_in_1):
            ...
            p_burst = material1['SF'] * p_vent  # burst pressure at which the tank fails according to Brewer book, w/o safety factor
            #print("t_sphe",(p_burst * r_in_1) / (2. * weld_eff * material1['stress_limit']  - 0.2 * p_burst))
        ...
        #print("VOL",calculateLh2MassToTankVolume(m_h2_rq, p_fill, y_max))
        ...
        rho_lh2 = np.interp(2.5, [1., 10.98], [70.83, 47.29])
        rho_gh2 = np.interp(2.5, [1., 10.73], [1.427, 15.898])
        rho_mean = y_max * rho_lh2 + (1. - y_max) * rho_gh2

        m_h2_rq += 0.15 * calculateLh2MassToTankVolume(m_h2_rq, p_fill, y_max) * rho_mean
        #print("add",0.15 * calculateLh2MassToTankVolume(m_h2_rq, p_fill, y_max) * rho_mean)
    ...
    def designThermalInsualtion(material1,material2,material3,material4,material5,material6,m_h2_rq,P_fc,vacuum,t_spaceneeded,t_added_vent, t_added_outlet, t_ins_old):
        ...
        D_outlet_out = (D_outlet_in + t_added_outlet * 2 + 2 * t_pipe) * np.ones(m_in.shape)
        ...
        # https://mid-mountain.com/wp-content/uploads/datasheets/Cermex%20PR1000-P%20Fiberglass%20Paper.pdf
...
#        A_outlet = m_dot_lh2_fc / rho_lh2 / v_lh2
 #       D_outlet_rq = ((A_outlet / np.pi) ** 0.5 + t_pipe * 2) #diameter we need for the FC
        ...
        m_dot_lh2_fc = c_discharge * rho_lh2 * np.pi * (D_outlet_in**2)/4. * (2 * (p_vent - 2.2e5) / rho_lh2) ** 0.5
        m_dot_lh2_ref = c_discharge * rho_lh2 * np.pi * (D_outlet_in**2)/4. * ((2 * ( 3.e5 - p_vent) / rho_lh2) ** 0.5)


        # A_inlet = m_lh2_in / rho_lh2 / v_lh2
        # D_inl = ((A_inlet / np.pi) ** 0.5 )
        # print("m_dot_lh2_ref",m_dot_lh2_ref)
        # print("m_dot_lh2_fc", m_dot_lh2_fc)




        # vent
        D_vent_in = 10e-3 * np.ones(A_m_cyl.shape)  # https://www.herose.com/eng/products/cryogenic-services/06810_06815.php
        D_vent_out = (D_vent_in + 2 * t_pipe + 2 * t_added_outlet) * np.ones(A_m_cyl.shape)
        A_vent = np.pi * ((D_vent_out / 2) ** 2 - (D_vent_in / 2) ** 2)

        v_gh2 = (2 * (p_vent - p_atm) / rho_gh2) ** 0.5  # speed of gh2 through vent https://en.wikipedia.org/wiki/Discharge_coefficient
        m_dot_gh2_vent = c_discharge *  rho_gh2 * A_vent * v_gh2
        Q_max = m_dot_gh2_vent * h_f_h2
        t_dump = (12.49/0.95/0.85)*0.03 / m_dot_gh2_vent
        ...
        A_pipe = np.pi * ((D_outlet_out/2)**2 - (D_outlet_in/2)**2) + np.pi * ((D_vent_out/2)**2 - (D_vent_in/2)**2)
        guess = 15
        n_layer = np.array([])
        ...
        for j in range(A_pipe.size):
            def findMinimumLayersMLI(x):  # minimum thickness finder
                ...
                Q_cond_pipe = material6["thermal_cond"] * (T_amb - T_lh2) * A_pipe[j] / (t_onelayer * x[0] + 0.35)
                Q_cond_support = material5["thermal_cond"] * (T_amb - T_lh2) * A_support * n_support / (
                        t_onelayer * x[0])
                Q_cond_MLI = (T_amb - T_lh2) * k_a * (A_m_cyl[j] + A_m_sphe[j]) / (t_onelayer * x[0])
                ...
                # emissivity_fac = 1. / ( (x[0] * 2. / material4["emissivity"]) - x[0] - 1 + 1. / material1["emissivity"] + 1. / material2["emissivity"] )
        ...
        t_ins += np.maximum(t_spaceneeded,np.zeros(m_MLI.shape))

        m_pipe = A_pipe * (t_ins +  r_in_2_cyl) * material6['rho_mat']
        m_support = A_support * t_ins * material5['rho_mat']
        m_radshield = 0.6  * m_MLI
        m_spacer =  0.4 * m_MLI
        ...
        return(m_MLI,t_ins,m_pipe,m_support,D_vent_in,D_outlet_in,m_radshield,m_spacer)
    ...
    def designOuterWall(material1,material2,t_ins):
        ...
        #
        # print("Wall cyl. thickness out = ", t_out_cyl * 1000, "mm")
        # print("Wall sph. thickness out = ", t_out_sphe * 1000, "mm")
    ...
    [m_ins,t_ins,m_pipe,m_support,D_vent_in,D_outlet_in,m_radshield,m_spacer] = designThermalInsualtion(material1, material2, material3, material4,material5, material6, m_h2_rq, P_fc, vacuum, np.zeros(m_in.shape),np.ones(m_in.shape)*0.0028,np.ones(m_in.shape)*0.0028,np.zeros(m_in.shape))
    [m_out,r_out_2_cyl,r_out_2_sphe,t_out_cyl] = designOuterWall(material1, material2, t_ins)


    [T_n_1, T_s_in_vent, x_1, y_in_vent, m_pipe_vent_in] = designOpeningReinforcement(D_vent_in,t_in_cyl,t_pipe,material1,material6)
    [T_n_2, T_s_out_vent, x_2, y_out_vent, m_pipe_vent_out] = designOpeningReinforcement(D_vent_in,t_out_cyl,t_pipe,material1,material6)
    [T_n_3, T_s_in_outlet, x_3, y_in_outlet, m_pipe_outlet_in] = designOpeningReinforcement(D_outlet_in,t_in_cyl,t_pipe,material1,material6)
    [T_n_4, T_s_out_outlet, x_4, y_out_outlet, m_pipe_outlet_out] = designOpeningReinforcement(D_outlet_in,t_out_cyl,t_pipe,material1,material6)

    # print("rad", r_out_2_sphe )
    # print("length", r_in_1*lamb)
    # print("tot", r_out_2_sphe*2+ r_in_1*lamb)
    ...
    t_spaceneeded = -np.minimum(t_spaceneeded, np.zeros(t_spaceneeded_outlet.shape))

    t_added_vent = abs(np.maximum(T_n_1,T_n_2) - t_pipe)
    t_added_outlet = abs(np.maximum(T_n_3,T_n_4) - t_pipe)
    #print(t_added_vent)

    if t_spaceneeded[0] > 0:
        for i in range(2):
            [m_ins,t_ins,m_pipe,m_support,D_vent_in,D_outlet_in,m_radshield,m_spacer] = designThermalInsualtion(material1, material2, material3, material4,material5, material6, m_h2_rq, P_fc, vacuum, t_spaceneeded, t_added_vent, t_added_outlet, t_ins)

            [m_out, r_out_2_cyl, r_out_2_sphe, t_out_cyl] = designOuterWall(material1, material2, t_ins)

            [T_n_2, T_s_out_vent, x_1, y_out_vent, m_pipe_vent_out] = designOpeningReinforcement(D_vent_in, t_out_cyl, t_pipe,material1, material6)
            [T_n_4, T_s_out_outlet, x_2, y_out_outlet, m_pipe_outlet_out] = designOpeningReinforcement(D_outlet_in, t_out_cyl, t_pipe, material1, material6)
    ...
    # print(y_out_vent,y_out_outlet,y_in_outlet,y_in_vent)
    # print(T_n_1,T_n_2,T_n_3,T_n_4)
    ...
    # # Check
    # print("min_thick",calculatePipeThickness(3e5,D_outlet_in,stain_steel))
    # print("min_thick", calculatePipeThickness(p_vent, D_outlet_in, stain_steel))
    ...
    # print("r_out_2_cyl",r_out_2_cyl*2*1000)
    # print("r_out_2_sph", r_out_2_sphe*2*1000)
    # print("length",lamb*r_in_1+2*r_out_2_sphe)

    m_tot = np.array([m_tot])
    m_h2_rq = np.array([m_h2_rq])

    return(m_tot, m_h2_rq, m_in, m_radshield, m_spacer, m_out, m_support)
...
def progress_wrapped(estimated_time, tstep=0.2, tqdm_kwargs={}):
    ...
    def real_decorator(function):
        ...
        def wrapper(*args, **kwargs):
            return provide_progress_bar(function, estimated_time=estimated_time, tstep=tstep,
                                        tqdm_kwargs=tqdm_kwargs, args=args, kwargs=kwargs)
...
if __name__ == "__main__":
    ...
    m_h2_flight = [12.492943560772767,15]
    m_h2_flight = np.array([m_h2_flight])
    P_fc = [1e3]
    P_fc = np.array([P_fc])

    retval = provide_progress_bar(designMechanicalTank,
                                  estimated_time=10, tstep=1 / 5.0,
                                  tqdm_kwargs={
                                      "bar_format": '{desc}: {percentage:3.0f}%|{bar}| {n:.1f}/{total:.1f} [{elapsed}<{remaining}]'},
                                  args=(m_h2_flight, P_fc))
